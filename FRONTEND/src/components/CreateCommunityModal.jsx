...
const CreateCommunityModal = ({ onClose, onCommunityCreated, currentUser }) => {
  const [formData, setFormData] = useState({
    name: "",
    description: "",
    category: "",
    isPrivate: false, // This matches the Java model property name
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createCommunity(formData, currentUser?.token);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md mx-4">
        <div className="flex justify-between items-center p-4 border-b dark:border-gray-700">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white">Create New Community</h2>
          <button
            onClick={onClose}
